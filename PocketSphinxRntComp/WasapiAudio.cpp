...
#define MY_MAX_RAW_BUFFER_SIZE 1024*128 // increase if missing peaces of sound
...
bool WasapiAudio::StopAudioCapture()
{
	...
	// TODO performance:
	// Split all to 3200 length buffers and calculate volume
	// return as list with custom List<buffer & volume>
...
void WasapiAudio::SkipFiveSecs()
{
	...
	int newAudioIndex = audioIndex + 2 * CHANNELS * SAMPLERATE * 5;
...
HRESULT WasapiAudio::InitCapture()
{
    ...
    if (SUCCEEDED(hr))
    {
		...
		MyFillPcmFormat(temp, CHANNELS, SAMPLERATE, BITS); // stereo, 44100 Hz, 16 bit
...
HRESULT WasapiAudio::InitRender()
{
    ...
    if (SUCCEEDED(hr))
    {
        WAVEFORMATEX temp;
        //MyFillPcmFormat(temp, 2, 44100, 16); // stereo, 44100 Hz, 16 bit
		MyFillPcmFormat(temp, CHANNELS, SAMPLERATE, BITS);
